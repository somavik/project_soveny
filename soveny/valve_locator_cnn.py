...
# ─────────────────────────────────────────────
# 2. Modell architektúra — Dilated 1D-CNN (TCN stílusú)
#
#  Miért CNN a BiLSTM helyett?
#  • A szelep pozícióját LOKÁLIS geometria határozza meg
#    (curvature ugrás, Vent_dist változás) — nem kell "emlékezni"
#    arra, mi volt 80 lépéssel ezelőtt.
#  • Dilated convolution: exponenciálisan növekvő "látótávolság"
#    dilation=1,2,4,8,16 → receptive field ≈ 63 pont (100-ból)
#    → ugyanolyan globális kontextus, mint a BiLSTM, de egyszerűbben.
#  • ~50%-kal kevesebb paraméter → kevesebb overfitting 176 mintánál.
#  • Párhuzamosítható → gyorsabb tanítás.
# ─────────────────────────────────────────────

class _ResidualDilatedBlock(nn.Module):
    """
    Egy dilated Conv1d blokk reziduális kapcsolattal:
      x → Conv(dilation=d) → GELU → Dropout → Conv(1×1) → + x
    A padding='same' ekvivalens: padding = dilation * (kernel-1)//2
    """
    def __init__(self, channels: int, dilation: int, dropout: float = 0.2):
        super().__init__()
        pad = dilation  # kernel_size=3 esetén: dilation*(3-1)//2 = dilation
        self.block = nn.Sequential(
            nn.Conv1d(channels, channels, kernel_size=3, padding=pad, dilation=dilation),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Conv1d(channels, channels, kernel_size=1),   # channel-mixing
        )
        self.norm = nn.GroupNorm(num_groups=min(8, channels), num_channels=channels)

    def forward(self, x):
        return self.norm(x + self.block(x))  # reziduális + normalizáció


class ValveLocatorCNN(nn.Module):
    """
    Seq2Seq dilated CNN:
      Bemenet : (Batch, 100, input_dim)   — (B, SeqLen, Features)
      Kimenet : (Batch, 100)              — logits minden pozícióhoz

    Receptive field számítás (kernel=3, dilations=[1,2,4,8,16]):
      RF = 1 + 2 * sum(dilations) = 1 + 2*(1+2+4+8+16) = 63 pont
    """
    def __init__(self, input_dim: int = 8, channels: int = 32, dropout: float = 0.2):
        super().__init__()
        ...
        # 5 dilated blokk: exponenciálisan növekvő látótávolság
    ...
    def forward(self, x):
        # x: (B, 100, 8)
        x = x.transpose(1, 2)      # (B, 8, 100)  — CNN channels-first
...
if __name__ == "__main__":
    ...
    diagnose_model(data_path="training_data_lv_aorta.npz")